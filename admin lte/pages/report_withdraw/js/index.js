...
async function fetchWithdraw(start_date, end_date) {
    ...
    let data = [];
    let tableData = [];
    let resWithdrawReport = await callXMLHttpRequest(`${apiPort.apiQueryWithdrawReport}`, { start_date: start_date, end_date: end_date });
...
async function fetchWithdrawAuto(start_date, end_date) {
    let column = [{
            title: "ชื่อผู้ใช้งาน",
            className: "align-middle"
        },
    ...
    let data = [];
    let tableData = [];
    let resWithdrawReport = await callXMLHttpRequest(`${apiPort.apiQueryWithdrawAutoReport}`, { start_date: start_date, end_date: end_date });
    if (resWithdrawReport.statusCodeText == textRespone.ok.CodeText) {
        data = resWithdrawReport.withdraw;
        for (let i = 0; i < data.length; i++) {
            tableData.push([
    ...
    initDataTables(tableData, 'tb_deposit_auto', column);
}