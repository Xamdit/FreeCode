...
async function fetchWithdraw(start_date, end_date) {
    ...
    let data = [];
    let tableData = [];
    let resWithdrawReport = await callXMLHttpRequest(`${apiPort.apiQueryPowYingShupReport}`, { start_date: start_date, end_date: end_date });
    if (resWithdrawReport.statusCodeText == textRespone.ok.CodeText) {
        ...
        for (let i = 0; i < data.length; i++) {
            tableData.push([
                ...
                data[i].credit,
                data[i].result == 0 ? '<span style="color:red">แพ้</span>' : data[i].result == 1 ? '<span style="color:#f9b10f">เสมอ</span>' : '<span style="color:green">ชนะ</span>',
